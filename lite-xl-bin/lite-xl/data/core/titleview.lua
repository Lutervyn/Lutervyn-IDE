...
function TitleView:update()
  ...
  if self.visible then 
     ...
     if style.logo_image then
       logo_w = style.logo_image:get_width() + style.padding.x * 2
     else
...
function TitleView:draw_window_title()
  ...
  if style.logo_image then
    local logo_w = style.logo_image:get_width()
    local logo_h = style.logo_image:get_height()
    
    -- Center logo vertically in the title bar
    local img_y = y + (h - logo_h) / 2
    renderer.draw_image(style.logo_image, x, img_y)
