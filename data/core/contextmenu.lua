...
function ContextMenu:on_selected(item)
  if type(item.command) == "string" then
    core.add_thread(function() coroutine.yield(); command.perform(item.command) end)
  else
    core.add_thread(function() coroutine.yield(); item.command() end)
  end
end
...
function ContextMenu:draw_context_menu()
  ...
  local shadow_color = { common.color "#000000" }
  shadow_color[4] = 60 -- Semi-transparent
  renderer.draw_rect(bx + 4 * SCALE, by + 4 * SCALE, bw, bh, shadow_color)
