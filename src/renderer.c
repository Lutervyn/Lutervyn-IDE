...
RenImage* ren_image_load(const char *filename) {
  ...
  /* Make a copy so we own the pixel data in SDL format */
  SDL_Surface *owned = SDL_ConvertSurface(surface, SDL_PIXELFORMAT_RGBA32);
  SDL_DestroySurface(surface);
  stbi_image_free(data);
  if (!owned) return NULL;

  RenImage *image = check_alloc(SDL_calloc(1, sizeof(RenImage)));
  image->surface = owned;
  image->width = w;
  image->height = h;
  return image;
...
void ren_image_free(RenImage *image) {
  if (image) {
...
int ren_image_get_width(RenImage *image) {
  return image->width;
}

int ren_image_get_height(RenImage *image) {
  return image->height;
}

void ren_draw_image(RenSurface *rs, RenImage *image, RenRect dest_rect) {
  ...
  SDL_Rect dst = {
    ...
    dest_rect.width * surface_scale,
    dest_rect.height * surface_scale
  };

  SDL_SetSurfaceBlendMode(image->surface, SDL_BLENDMODE_BLEND);
  SDL_BlitSurfaceScaled(image->surface, NULL, surface, &dst, SDL_SCALEMODE_LINEAR);
}
