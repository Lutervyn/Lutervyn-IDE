...
static int f_image_load(lua_State *L) {
  const char *filename = luaL_checkstring(L, 1);
  RenImage** image = lua_newuserdata(L, sizeof(RenImage*));
  *image = ren_image_load(filename);
  if (!*image)
...
static int f_image_get_width(lua_State *L) {
  RenImage** image = luaL_checkudata(L, 1, API_TYPE_IMAGE);
  lua_pushnumber(L, ren_image_get_width(*image));
  return 1;
}

static int f_image_get_height(lua_State *L) {
  RenImage** image = luaL_checkudata(L, 1, API_TYPE_IMAGE);
  lua_pushnumber(L, ren_image_get_height(*image));
  return 1;
}
...
static int f_draw_image(lua_State *L) {
  ...
  lua_Number w = luaL_optnumber(L, 4, ren_image_get_width(*image));
  lua_Number h = luaL_optnumber(L, 5, ren_image_get_height(*image));
  RenRect rect = rect_to_grid(x, y, w, h);
  rencache_draw_image(ren_get_target_window(), *image, rect);
  return 0;
}
...
static const luaL_Reg imageLib[] = {
  { "__gc",               f_image_gc                },
  { "load",               f_image_load              },
  { "get_width",          f_image_get_width         },
  { "get_height",         f_image_get_height        },
